...
export async function user(userId: number) {
  ...
  _user.post("/sendMessage", async (req, res) => {
    ...
    let message2Send = lastSentMessage;
    for (let i = circuit.length - 1; i >= 0; i--) {
      ...
      const message2Encrypt = `${destination + message2Send}`;
      const encryptedMessage = await symEncrypt(symKey, message2Encrypt);
      const encryptedSymKey = await rsaEncrypt(await exportSymKey(symKey), node.pubKey);
      message2Send = encryptedSymKey + encryptedMessage;
    }
    ...
    await fetch(`http://localhost:${BASE_ONION_ROUTER_PORT + entryNode.nodeId}/message`, {
      method: "POST",
      body: JSON.stringify({ message: message2Send }),
      headers: {
